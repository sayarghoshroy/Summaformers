...
class CNN_RNN_Regression(BasicModule):
    def __init__(self, args, embed=None):
        super(CNN_RNN_Regression,self).__init__(args)
        print("In CNN_RNN_Regression.py")
        self.model_name = 'CNN_RNN_Regression'
        self.args = args
    ...
    def forward(self,x,doc_lens):
        ...
        for index,doc_len in enumerate(doc_lens):
            ...
            for position, h in enumerate(valid_hidden):
                ...
                prob = act(content + salience + novelty + abs_p + rel_p + self.bias)
                # print(probs)
                s = s + torch.mm(prob,h)
                probs.append(prob)
        # print(sorted(torch.cat(probs).squeeze()))
